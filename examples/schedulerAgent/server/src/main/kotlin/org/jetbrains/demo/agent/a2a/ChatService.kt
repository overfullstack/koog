...
class ChatService(
    private val orchestrator: SchedulerOrchestratorAgent,
    ...
    private fun extractAppointmentDetails(message: String, current: PartialAppointmentInfo): PartialAppointmentInfo {
        ...
        val datePatterns = listOf(
            ...
            // Text dates like "January 12", "Jan 12", "12 January", "12th January"
            Regex("(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)\\s+\\d{1,2}(?:st|nd|rd|th)?(?:,?\\s+\\d{4})?", RegexOption.IGNORE_CASE),
            Regex("\\d{1,2}(?:st|nd|rd|th)?\\s+(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)(?:,?\\s+\\d{4})?", RegexOption.IGNORE_CASE)
        )
    ...
    private fun parseAppointmentDateTime(dateStr: String?, timeStr: String?): LocalDateTime? {
        ...
        // Try parsing text dates like "January 12", "Jan 12", "12th", etc.
        dateStr?.let { date ->
            val dateLower = date.lowercase().trim()
            // Pattern: "January 12" or "Jan 12" or "12 January" etc.
            ...
            // Try patterns like "January 12" or "Jan 12"
            for ((monthName, monthNum) in monthNames) {
                val pattern1 = Regex("""$monthName\s+(\d{1,2})(?:st|nd|rd|th)?(?:,?\s+(\d{4}))?""", RegexOption.IGNORE_CASE)
                val match1 = pattern1.find(dateLower)
                if (match1 != null) {
                    val day = match1.groupValues[1].toIntOrNull() ?: continue
                    val year = match1.groupValues[2].toIntOrNull() ?: now.year
                    val parsedDate = LocalDateTime(year, monthNum, day, 0, 0)
                    return parseTime(timeStr, parsedDate) ?: LocalDateTime(year, monthNum, day, 14, 0)
                }
                
                // Try patterns like "12 January" or "12th January"
                val pattern2 = Regex("""(\d{1,2})(?:st|nd|rd|th)?\s+$monthName(?:,?\s+(\d{4}))?""", RegexOption.IGNORE_CASE)
                val match2 = pattern2.find(dateLower)
                if (match2 != null) {
                    val day = match2.groupValues[1].toIntOrNull() ?: continue
                    val year = match2.groupValues[2].toIntOrNull() ?: now.year
                    val parsedDate = LocalDateTime(year, monthNum, day, 0, 0)
                    return parseTime(timeStr, parsedDate) ?: LocalDateTime(year, monthNum, day, 14, 0)
                }
            }
            
            // Try pattern like "12th" (assume current month and year)
            val dayOnlyPattern = Regex("""(\d{1,2})(?:st|nd|rd|th)?""")
            val dayOnlyMatch = dayOnlyPattern.find(dateLower)
            if (dayOnlyMatch != null && !dateLower.contains("jan") && !dateLower.contains("feb") && 
                !dateLower.contains("mar") && !dateLower.contains("apr") && !dateLower.contains("may") &&
                !dateLower.contains("jun") && !dateLower.contains("jul") && !dateLower.contains("aug") &&
                !dateLower.contains("sep") && !dateLower.contains("oct") && !dateLower.contains("nov") &&
                !dateLower.contains("dec")) {
                val day = dayOnlyMatch.groupValues[1].toIntOrNull()
                if (day != null && day >= 1 && day <= 31) {
                    val parsedDate = LocalDateTime(now.year, now.monthNumber, day, 0, 0)
                    return parseTime(timeStr, parsedDate) ?: LocalDateTime(now.year, now.monthNumber, day, 14, 0)
                }
            }
        }
        
        // Try parsing ISO format or numeric dates
        return try {
