...
private fun appointmentBookingAgent(
    ...
    val toolRegistry = ToolRegistry {
        tools(org.jetbrains.demo.agent.tools.AppointmentBookingTool())
    }
...
private fun appointmentBookingStrategy() = strategy<A2AMessage, Unit>("appointment-booking-strategy") {
    ...
    val bookAppointment by node<AppointmentBookingRequest, Pair<AppointmentBookingRequest, String>> { request ->
        ...
        if (timeslotInfo == null) {
            logger.warn("${LogColors.APPOINTMENT_BOOKING} No timeslot info provided, skipping booking")
            return@node Pair(request, """{"error": "No timeslot information available", "success": false}""")
        }
        
        // Get service territory ID from the request - we need to extract it from the appointment form or pass it through
        // For now, we'll need to get it from the orchestrator. Let me check if we need to add it to the request.
        // Actually, we need serviceTerritoryId - let me check the AppointmentBookingRequest structure
        val bookingTool = org.jetbrains.demo.agent.tools.AppointmentBookingTool()
        
        // We need serviceTerritoryId - it should be passed from the orchestrator
        // For now, let's assume it needs to be added to the request or we extract it from context
    ...
    val generateConfirmation by node<Pair<AppointmentBookingRequest, String>, AppointmentBookingResult> { (request, bookingResponse) ->
        ...
        val bookingSuccess = try {
            ...
            if (bookingJson is JsonObject) {
                bookingJson["success"]?.jsonPrimitive?.content?.toBoolean() ?: false
            } else {
                false
            }
        } catch (e: Exception) {
                ...
                user {
                    markdown {
                        ...
                        bulleted {
                            ...
                            if (bookingSuccess) {
                                item("The appointment has been successfully booked via the API")
                                item("Extract the appointment ID from the booking response if available")
                            } else {
