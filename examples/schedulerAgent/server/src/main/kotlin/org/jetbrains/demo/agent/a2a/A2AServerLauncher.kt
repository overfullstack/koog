...
fun main() = runBlocking {
    logger.info("Starting A2A Scheduler Servers...")

    val config = A2AConfig(
        baseUrl = System.getenv("A2A_BASE_URL") ?: "http://localhost",
        locationWeatherPort = System.getenv("LOCATION_WEATHER_PORT")?.toIntOrNull() ?: 9101,
    ...
    logger.info("A2A Scheduler servers started successfully!")
    logger.info("Endpoints:")
    logger.info("  Location Weather:  ${config.baseUrl}:${config.locationWeatherPort}$LOCATION_WEATHER_PATH")
